...
def make_fig2(fig_path, n_m1s, n_m2s):
    ...
    for i in tqdm(range(thetas.shape[0])):
        densities_analytic[i] = np.sqrt(np.linalg.det(analytic_metric(fs, thetas[i], Sn_np)))
    ...
    plt.colorbar(cs, label=r"$\log_{10} \left(\frac{\sqrt{|g^ {ad}|}-\sqrt{|g|}}{\sqrt{|g|}}\right)$").ax.tick_params(axis='y', direction='in')
