...
# From https://bitbucket.org/dailiang8/gwbinning/src/master/waveform.py
...
# step function
@jit
def myHeaviside(x: jnp.ndarray) -> jnp.ndarray:
...
def Psi(
    ...
    M: jnp.ndarray,
    eta: jnp.ndarray,
    s1x: jnp.ndarray = jnp.array(0.0),
    s1y: jnp.ndarray = jnp.array(0.0),
    s1z: jnp.ndarray = jnp.array(0.0),
    s2x: jnp.ndarray = jnp.array(0.0),
    s2y: jnp.ndarray = jnp.array(0.0),
    s2z: jnp.ndarray = jnp.array(0.0),
    Lam: jnp.ndarray = jnp.array(0.0),
    dLam: jnp.ndarray = jnp.array(0.0),
):
    ...
    phis_3PN = pi * (3760.0 * SL + 1490.0 * dSigmaL) / 3.0 + pn_ss3
    phis_35PN = (
    ...
    return (
        ...
        * (
            ...
            * v4
            + (38645.0 / 756.0 * pi - 65.0 / 9.0 * eta * pi - ga)
            * (1.0 + 3.0 * jnp.log(v / vlso))
            * v5
            + (
                11583231236531.0 / 4694215680.0
                - 640.0 / 3.0 * pi ** 2
                - 6848.0 / 21.0 * (jnp.euler_gamma + jnp.log(4.0 * v))
...
def amp(
    ...
    return (
        ...
        * (
        ...
        + v5 * (57.0 / 16.0 * pi * eta - 4757.0 * pi / 1344.0 + eps)
        + v6
        * (
            856.0 / 105.0 * jnp.euler_gamma
...
#     return pre * A0 * A * jnp.exp(-1.0j * Phi)
#
#
...
#     r0, r1 = rdata
#     denom = jnp.sum(
#         sdat[2][k] * jnp.absolute(r0) ** 2
#         + sdat[3][k] * 2.0 * (r0 * jnp.conjugate(r1)).real
#     )
#     X = jnp.sum(sdat[0][k] * jnp.conjugate(r0) + sdat[1][k] * jnp.conjugate(r1))
#     res = jnp.absolute(X) ** 2 / denom
#     # print('denom = ', denom)
#     # print('overlap = ', res)
#     return res
#
...
#     # waveform ratio
#     r = (
#         hf3hPN(f, M, eta, s1z=s1z, s2z=s2z, Lam=Lam)
#         / h0_bin
#         * jnp.exp(-2.0j * pi * f * dtc)
#     )
#     r0 = 0.5 * (r[:-1] + r[1:])
#     r1 = (r[1:] - r[:-1]) / (f[1:] - f[:-1])
#
#     return jnp.array([r0, r1], dtype=jnp.complex128)
#
#
