...
"""
4dv2 waveform metric parametrized by the black hole masses (m1, m2).
"""


def Psi(f: jnp.ndarray, theta: jnp.ndarray) -> jnp.ndarray:

    mass1, mass2, chiz, kappa2PN_eff = theta
    ...
    # q= mass2/mass1
    # g=2*(1+q)**2.0 / (1+q**2.0)
    # kappa3PN_eff = g*kappa2PN_eff

    tau0 = (
        12769
        * (250069 - 32 * eta * (20941 - 1805 * eta))
        / (20 * ((113 - 76 * eta) ** 2) * (8475 + 1444 * eta))
    )
    kappa3PN_eff = (2 / (1 - 2 * eta)) * kappa2PN_eff + (1 / (1 - 2 * eta)) * (
        2 * tau0 * chiz ** 2.0
    )
    ...
    rho0 = 113.0 * jnp.pi / (113.0 - 76.0 * eta) * (2270.0 / 3.0 - 520.0 * eta)
    rho1 = (
