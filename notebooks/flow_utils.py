...
def get_nice_transform(hidden_dim=64):
    def transform(rng, input_dim, output_dim):
        params, apply_fun = get_transform(hidden_dim=hidden_dim)(
...
def get_conv_transform(hidden_dim=64, act=stax.Relu):
    def Reshape():
        def init_fun(rng, input_shape):
            return (
                ...
                14,
                1,
            ), ()  # round(input_shape[1] ** 0.5), round(input_shape[1] ** 0.5), 1), ()

        def apply_fun(params, inputs, **kwargs):
            return inputs.reshape(
                (inputs.shape[0], 28, 14, 1)
            )  # round(input_shape[1] ** 0.5), round(input_shape[1] ** 0.5), 1)
    ...
    def transform(rng, input_dim, output_dim):
        init_fun, apply_fun = stax.serial(
            ...
            stax.Flatten,
            stax.Dense(
                output_dim, W_init=weight_initializer, b_init=weight_initializer
            ),
        )
        _, params = init_fun(rng, (input_dim,))
        return params, apply_fun
